...
from style import apply_style, PALETTE, MARKERS, RESULTS_DIR, save_fig
...
PRELIMINARY = [
    {
        'label': 'Llama-1B', 'family': 'Llama',
        'params_b': 1.236,
        'pcorr_mean': 0.250, 'pcorr_std': 0.002,
        'source': 'cross_family.json (3 seeds, 150 ex/dim)',
    },
    {
        'label': 'Llama-8B', 'family': 'Llama',
        'params_b': 8.0,
        'pcorr_mean': 0.088, 'pcorr_std': 0.004,
...
def main():
    ...
    for label, m in sorted(models.items(), key=lambda x: x[1]['params_b']):
        family = m['family']
        per_seed = m['partial_corr'].get('per_seed', [])
        yerr = np.std(per_seed) if len(per_seed) > 1 else 0
        points.append({
            'x': m['params_b'], 'y': m['partial_corr']['mean'],
            'yerr': yerr, 'family': family,
            'preliminary': False,
        })

    for p in PRELIMINARY:
        points.append({
            'x': p['params_b'], 'y': p['pcorr_mean'],
            'yerr': p['pcorr_std'], 'family': p['family'],
            'preliminary': True,
        })
    ...
    for p in points:
        ...
        ax.plot(
            p['x'], p['y'],
            MARKERS[fam], color=PALETTE[fam],
            markersize=5, zorder=3, label=legend_label, **mkw,
        )
