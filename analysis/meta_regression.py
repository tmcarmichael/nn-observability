...
from load_results import load_per_seed, load_all_models
...
def run_mixed_effects():
    ...
    if singletons:
        ...
        print(f'  These enter the model but contribute no seed-level information.\n')

    print('=== Data summary ===')
    for fam in sorted(df['family'].unique()):
        sub = df[df['family'] == fam]
        print(f'  {fam:<8}: {len(sub)} obs across {sub["model"].nunique()} models, '
    ...
    try:
        md = smf.mixedlm('partial_corr ~ log_params + C(family)',
                          df, groups=df['model'])
    ...
    var_between_family = sum(n * (m - grand_mean)**2
                             for m, n in zip(family_means, family_ns)) / len(df)
    ...
    if pct_seed > 30:
        ...
        print(f'  protocol. The mixed-effects model absorbs this correctly.')
