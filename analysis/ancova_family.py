...
def run_ancova():
    ...
    for fam in sorted(df['family'].unique()):
        ...
        print(f'  {fam:<8}: {len(sub)} obs, {sub["model"].nunique()} models, '
    ...
    for label, model in [('Intercept only', m1), ('+ log(params)', m2),
                          ('+ family', m3), ('+ family x scale', m4)]:
    ...
    if len(qwen_3b) > 0 and len(llama_3b) > 0:
        ...
        u, p = mannwhitneyu(qwen_3b['partial_corr'], llama_3b['partial_corr'],
                            alternative='greater')
        print(f'=== Scale-matched contrast: Qwen 3B vs Llama 3B ===')
        ...
        missing = []
        if len(qwen_3b) == 0: missing.append('Qwen-3B')
        if len(llama_3b) == 0: missing.append('Llama-3B')
