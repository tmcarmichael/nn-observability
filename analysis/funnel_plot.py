...
def run():
    ...
    for m in models:
        ...
        print(f'  {m["name"]:<15} {m["family"]:<8} {m["mean"]:+.4f} {m["std"]:.4f} '
    ...
    try:
        ...
        colors = {'GPT-2': '#4878A8', 'Qwen': '#E07B39',
                  'Llama': '#C44E52', 'Gemma': '#2CA02C'}
        plotted_families = set()
        for m in valid:
            label = m['family'] if m['family'] not in plotted_families else None
            ax.scatter(m['mean'], 1.0 / m['se'], c=colors.get(m['family'], 'gray'),
                       s=50, zorder=3, label=label)
            plotted_families.add(m['family'])

        ax.legend(fontsize=9)
        ax.axvline(x=np.mean(means_valid), color='gray', linestyle='--',
