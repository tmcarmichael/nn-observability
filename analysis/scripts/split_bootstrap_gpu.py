...
def train_and_eval(train_data, test_data, seed=42, epochs=20, lr=1e-3):
    ...
    rho, _ = partial_spearman(scores, test_data['losses'],
...
def main():
    ...
    for i in range(args.n_boot):
        ...
        print(f'  boot {i+1}/{args.n_boot}: pcorr={rho:+.4f}  '
              f'({dt:.0f}s, ~{remaining/60:.0f}m remaining) [{elapsed()}]')

        del train_data, boot_batches; gc.collect()
        if DEVICE == 'cuda': torch.cuda.empty_cache()

        # Checkpoint after each iteration
        _ckpt = {'model': args.model, 'peak_layer': peak, 'n_complete': i + 1,
                 'boot_rhos': boot_rhos, '_elapsed': elapsed()}
    ...
    out = {
        ...
        'n_boot': args.n_boot, 'boot_rhos': boot_rhos.tolist(),
        'mean': float(boot_rhos.mean()), 'std': float(boot_rhos.std()),
