...
from load_results import load_control_sensitivity, load_random_head_baselines, load_all_models
...
def analyze_selectivity():
    ...
    if baselines:
        for name, family, params, rh in baselines:
        ...
        print(f'  A randomly initialized probe with identical architecture achieves')
        print(f'  partial correlation indistinguishable from zero (mean {np.mean(rh_vals):+.4f}),')
        print(f'  confirming the trained probe finds learned structure.')
    else:
    ...
    if 'Llama' in fam_gaps and 'Qwen' in fam_gaps:
        ...
        if llama_gap > qwen_gap + 10:
            ...
            print(f'  Llama has less independent signal, not just less total signal.')
    ...
    if abs(np.mean(deltas)) < 0.02:
        print(f'  Nonlinear MLP does not reduce signal beyond linear controls.')
    else:
    ...
    for m in models:
        if m['standard'] > 0:
            ...
            print(f'  {m["name"]:<15}: {m["standard"]:+.4f} -> {m["plus_entropy"]:+.4f} '
