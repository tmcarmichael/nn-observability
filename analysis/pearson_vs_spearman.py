...
from load_results import load_control_sensitivity, load_all_models
...
def report():
    ...
    if abs(mean_delta) < 0.02:
        print(f'  Nonlinear raw-value controls match rank-based controls.')
        print(f'  Rank-transformation does not inflate reported effect sizes.')
    else:
        print(f'  Delta of {mean_delta:+.4f} between nonlinear and rank-based controls.')
        print(f'  Investigate before claiming rank-transformation is neutral.')
    print()
    print('Recommended paper text:')
    print(f'  "The nonlinear MLP control (operating on raw values) produces')
    print(f'  the same partial correlation as rank-based controls')
    print(f'  (mean delta {mean_delta:+.4f}), confirming rank-transformation')
    print(f'  does not inflate reported effect sizes."')
    print()
