...
def _load_gpt2() -> dict[str, dict[str, Any]]:
    ...
    for name, params_b, label in GPT2_MODELS:
        ...
        m = p8[name]
        models[label] = {
            'family': 'GPT-2', 'params_b': params_b, 'label': label,
    ...
    if cs and 'GPT2-124M' in models:
        models['GPT2-124M']['control_sensitivity'] = {
            k: v['mean'] for k, v in cs.items()
        }
    return models


def _load_family(file_list: list[tuple], family_name: str,
                 fallbacks: dict[str, str] | None = None) -> dict[str, dict[str, Any]]:
    ...
    for fname, params_b, label in file_list:
        ...
        if not pc or 'mean' not in pc:
            ...
            continue
        entry = {
            'family': family_name, 'params_b': params_b, 'label': label,
            'partial_corr': pc,
...
def load_all_models(verbose: bool = False) -> dict[str, dict[str, Any]]:
    ...
    if verbose:
        ...
        expected = ([(l, 'GPT-2') for _, _, l in GPT2_MODELS] +
...
def load_control_sensitivity() -> list[dict[str, Any]]:
    ...
    for label, m in models.items():
        ...
        if not cs or 'none' not in cs:
            continue
        results.append({
            'name': label, 'family': m['family'], 'params_b': m['params_b'],
            **{k: cs[k] for k in ['none', 'softmax_only', 'standard',
                                    'plus_entropy', 'nonlinear'] if k in cs},
        })
    return results
