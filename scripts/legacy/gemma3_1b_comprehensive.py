...
model = AutoModelForCausalLM.from_pretrained(MODEL_ID, dtype=torch.bfloat16, attn_implementation="sdpa").to(
    DEVICE
)
model.eval()
...
for layer in range(N_LAYERS):
    ...
    print(
        f"  L{layer:>2}: {rho:+.4f}  ({elapsed:.0f}s, ~{remaining / 60:.0f}m left, {elapsed_str()} elapsed)"
    )
    del tr, va
    gc.collect()
...
for layer in candidates:
    ...
    val_cache[layer] = collect_layer_data(
        model, tokenizer, wiki_val, layer, DEVICE, MAX_TRAIN, collect_entropy=True
    )
