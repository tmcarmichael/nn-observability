...
print(
    f"Approx tokens: {approx_train} train ({approx_train / HIDDEN_DIM:.0f} ex/dim), {approx_val} val ({approx_val / HIDDEN_DIM:.0f} ex/dim)"
)
...
for layer in candidate_layers:
    print(
        f"  L{layer} train: {len(train_cache[layer]['losses'])} ({len(train_cache[layer]['losses']) / HIDDEN_DIM:.0f} ex/dim)"
    )
    print(
        f"  L{layer} val:   {len(val_cache[layer]['losses'])} ({len(val_cache[layer]['losses']) / HIDDEN_DIM:.0f} ex/dim)"
    )
...
for layer in candidate_layers:
    ...
    print(
        f"  L{layer}: {np.mean(seed_rhos):+.4f} +/- {np.std(seed_rhos):.4f}  agree={np.mean(pairwise):+.4f}  seeds={[round(x, 4) for x in seed_rhos]}"
    )
...
for name, sc in hand_designed.items():
    rho_hd, _ = partial_spearman(
        sc, wiki_val_peak["losses"], [wiki_val_peak["max_softmax"], wiki_val_peak["activation_norm"]]
    )
...
rho_rand, _ = partial_spearman(
    rand_sc, wiki_val_peak["losses"], [wiki_val_peak["max_softmax"], wiki_val_peak["activation_norm"]]
)
...
print()
print(
    f"Val ex/dim: {len(wiki_val_peak['losses']) / HIDDEN_DIM:.0f} vs test ex/dim: {len(wiki_test_peak['losses']) / HIDDEN_DIM:.0f}"
)
print()
...
output = {
    ...
    "multi_layer_eval": {
        str(l): {
            ...
            "std": v["std"],
            "per_seed": v["per_seed"],
            "seed_agreement": v["seed_agreement"],
        }
    ...
    "version_history": {
        ...
        "v3": {
            "partial_corr": 0.214,
            "output_controlled": 0.096,
            "ex_per_dim": 350,
            "peak_layer": 30,
            "eval_split": "test",
            "test_ex_per_dim": 43.8,
        },
    },
}
