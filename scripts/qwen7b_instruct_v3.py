"""Qwen 2.5 7B Instruct: v3 with optimized GPU utilization.

Model: Qwen/Qwen2.5-7B-Instruct | GPU: H100 SXM 80GB | 7 seeds (43-49) | 350 ex/dim
...
Usage: pip install transformers datasets scipy && python scripts/qwen7b_comprehensive_v3.py
"""
...
BATCH_SIZE = 48
SM_CHUNK = 8  # chunk size for softmax/entropy (manages GPU memory for large vocab)
...
def pretokenize(docs, tokenizer, max_length=512):
    ...
    for doc in docs:
        ...
        if len(ids) >= 2: encoded.append(ids)
...
def collect_multi_layer_fast(model, batches, layers, max_tokens, device):
    ...
    for bi, (input_ids_cpu, attn_mask_cpu) in enumerate(batches):
        ...
        # Shifted positions for next-token prediction (all on GPU)
        ...
        # Softmax + entropy in chunks (avoids materializing full float32 probs at once)
        ...
        # Masked extraction: flat valid positions -> CPU
        ...
        for l in layers: captured.pop(l, None)
    ...
    # Concatenate and trim to max_tokens
...
CHECKPOINT_PATH = Path('/workspace/qwen7b_instruct_v3_checkpoint.json')
...
LAYERS_PER_PASS = 1
print(f'Token budget: {MAX_TRAIN} train ({TARGET_EX_PER_DIM} ex/dim), {LAYERS_PER_PASS} layers/pass, batch={BATCH_SIZE}')
...
layer_profile = {}
layer_chunks = [list(range(i, min(i + LAYERS_PER_PASS, N_LAYERS))) for i in range(0, N_LAYERS, LAYERS_PER_PASS)]
...
for chunk in layer_chunks:
    ...
    del tr_multi, va_multi; gc.collect()
    if DEVICE == 'cuda': torch.cuda.empty_cache()
...
save_checkpoint('1_sweep', model=MODEL_ID, n_layers=N_LAYERS, hidden_dim=HIDDEN_DIM,
                layer_profile={str(k): v for k, v in sorted(layer_profile.items())},
                peak_layer=peak_layer, candidates=candidates)
...
save_checkpoint('3_multiseed', model=MODEL_ID, n_layers=N_LAYERS, hidden_dim=HIDDEN_DIM,
                layer_profile={str(k): v for k, v in sorted(layer_profile.items())},
                peak_layer_final=FINAL, peak_layer_frac=round(FINAL / N_LAYERS, 2),
...
print(f'\n=== Saving [{elapsed_str()}] ===')
output = {
    'model': MODEL_ID, 'n_params_b': round(n_params, 2),
    'n_layers': N_LAYERS, 'hidden_dim': HIDDEN_DIM,
    'protocol': {'layer_select_seed': LAYER_SELECT_SEED, 'eval_seeds': EVAL_SEEDS,
                 'target_ex_per_dim': TARGET_EX_PER_DIM, 'batch_size': BATCH_SIZE,
...
out_path = Path('/workspace/qwen7b_instruct_v3_results.json')
